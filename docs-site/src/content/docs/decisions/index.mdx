...
description: All 11 Architecture Decision Records documenting significant technical decisions.
---
...
## ADR 0011: Sub-Agent Nesting Resolution <Badge text="Accepted" variant="success" />

**Problem**: GitHub Copilot limits sub-agent nesting to 1 level. Conductor → L1 → L2 chains violate this.

**Decision**: Convert L2 sub-agents to skills (security-review, init-config, init-docs, init-scaffold). Standalone agent files kept for direct invocation.

**Affected chains**: init → (init-config, init-docs, init-scaffold), implement → security, plan → security, review → security, sprint → refine.
