...
<CardGrid>
  ...
  <Card title="Azure DevOps" icon="list-format">
    **Transport**: HTTP (`https://mcp.dev.azure.com/`)
    ...
    | Toolset | Purpose |
    |---------|---------|
    | `wit` | Work item CRUD, hierarchy management, traceability links, board management |
    | `work` | Sprint planning, team iterations, team capacity |
    | `search` | Duplicate detection, backlog queries |
    | `repos` | PR creation, review threads, branch management for ADO-hosted repos |
