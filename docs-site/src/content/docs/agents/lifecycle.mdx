...
Uses the `sdd-init.sh` script for deterministic file operations (verify, create, update, diff).
...
- Coordinator workflow can delegate context loading and architecture analysis to nested workers before authoring design artifacts
...
<Aside type="caution">
  Missing ADRs block the Foundational phase. Tests are integrated as acceptance criteria — no separate test tasks.
</Aside>
