...
The default limit is 3 tasks in progress. The developer can ignore it, but the system flags it. Developers should work on **one task at a time** ([ADR 0009](/decisions/#adr-0009-developer-capacity-)); the framework enforces this via workflow.

## Impact-Based Ceremony

Not every change needs all guardrails. The framework classifies tasks by [impact](/concepts/impact-classification/) and adjusts the level of ceremony:
...
Each phase transition includes a structured cleanup that preserves key decisions while resetting working context. See [Phase Cleanup Points](/core-components/context-management/) for the full sequence.
...
<CardGrid>
  <Card title="Team Coordination" icon="group">
    Multi-developer guardrails for parallel sessions and ADR synchronization. See [Team Coordination](/guardrails/team-coordination/).
  </Card>
  <Card title="Comprehension Checkpoints" icon="approve-check">
    How the agent verifies understanding before and after code generation. See [Comprehension Checkpoints](/concepts/comprehension-checkpoints/).
  </Card>
</CardGrid>
...
- [Impact Classification](/concepts/impact-classification/) for how changes are classified
- [Context Management](/core-components/context-management/) for how context flows between phases
