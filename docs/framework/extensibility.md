...
Workspace agent overrides (generated by the sync script) resolve MCP servers from `.vscode/mcp.json`, not from the plugin's `.mcp.json`. This means the consumer must configure the MCP server in `.vscode/mcp.json` even if the plugin already provides it.

This applies to two scenarios:

1. **New external servers** (Confluence, Jira, Slack): add the server config with credentials
2. **Plugin-provided servers** (github, azure, microsoft-learn): copy the server entry from the plugin's `.mcp.json` to `.vscode/mcp.json`

The `devsquad.extend` agent handles this automatically during scaffolding by reading the plugin's `.mcp.json` and mirroring the relevant entries.
