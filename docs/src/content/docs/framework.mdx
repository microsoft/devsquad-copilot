...
description: Design principles, architecture patterns, and usage scenarios. How small teams achieve fast, structured delivery with AI agents.
banner:
  content: |
...
<CardGrid stagger>
  <Card title="Socratic by Design" icon="approve-check">
  ...
  <Card title="Human-in-the-loop by Impact" icon="warning">
  ...
  <Card title="Specs are Living Artifacts" icon="pencil">
    Specs and ADRs are refined as implementation reveals new understanding. The framework has a named seam for amending them mid-flight, so discoveries made during the work update the shared model instead of being silently absorbed or lost. See [Spec Amendment](../concepts/spec-amendment/).
  </Card>
  <Card title="Flexibility" icon="random">
  ...
  <Card title="Context Isolation by Default" icon="document">
  ...
  <Card title="Principle of Least Privilege" icon="setting">
  ...
  <Card title="Trusted MCP Servers Only" icon="approve-check">
  ...
  <Card title="Decisions as Transparent Artifacts" icon="document">
  ...
  <Card title="Extensibility without Forking" icon="translate">
  ...
  <Card title="Stack and Model Agnostic" icon="setting">
...
The central `devsquad` agent implements a **Mediated Coordinator-Worker** pattern ([ADR 0001](../decisions/#adr-0001-agent-orchestration-)):
...
Sub-agents operate in two modes via a prefix-based protocol ([ADR 0002](../decisions/#adr-0002-communication-protocol-)):
...
Phases execute sequentially or across sessions. Context isolation prevents assumption contamination ([ADR 0003](../decisions/#adr-0003-context-management-)):
...
|-----------|-------|-------------|
| [Agents](../agents/overview/) | 13 visible + internal workers | One conductor, 12 specialist agents, and hidden worker sub-agents for nested execution |
| [Skills](../skills/) | 18 | Reusable capabilities loaded on demand |
| [Instructions](../core-components/instructions/) | 7 | Path-specific behavioral rules |
| [Hooks](../core-components/hooks/) | 5 | Lifecycle hooks for deterministic operations |
| [MCP Servers](../core-components/mcp-servers/) | 5 | Remote servers for external integrations |
| [Context Management](../core-components/context-management/) | - | Memory, handoff envelopes, disk persistence |
| [Extensibility](../extensibility/) | 6 | Extension mechanisms for customization |

### Extension Mechanisms

Four mechanisms extend agent capabilities without modifying agent code. Each has a different activation model ([ADR 0004](../decisions/#adr-0004-activation-model-)):
...
Dashed arrows: auto-activated (agent does not explicitly request). Solid arrows: agent-initiated. See [Extensibility](../extensibility/) for the full decision tree.
...
:::note
Changes to upstream artifacts (specs, ADRs, priorities) are not automatically propagated to downstream work items. See [Team Coordination](../guardrails/team-coordination/#change-propagation-limitations) for the full limitation matrix and recommended manual steps.
:::

See [Team Coordination](../guardrails/team-coordination/) for multi-developer guardrails.
...
<CardGrid>
  <LinkCard title="For Developers" href="../getting-started/" description="Install the plugin and run your first guided workflow." />
