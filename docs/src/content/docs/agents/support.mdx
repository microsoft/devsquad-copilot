...
## devsquad.refine <Badge text="Health" variant="note" /> <Badge text="Amendment" variant="tip" />

Analyze backlog health, detect inconsistencies between specs, ADRs, and work items, and apply scoped spec/ADR amendments when implementation reveals drift.

| | |
|---|---|
| **Produces** | Analysis report with fixes, or scoped spec/ADR amendment |
| **Handoff** | to specify, plan, decompose, or kickoff |
| **Skills** | documentation-style, reasoning, work-item-creation |

Two modes:

- **Backlog health** (default): between-sprint analysis across six categories.
- **Spec Amendment** (triggered by `[AMEND]` prompt prefix from `devsquad.implement`): scoped, surgical update to a single spec/ADR section when the implement agent raises a `spec-drift` flag. See [Spec Amendment During Implementation](../../concepts/spec-amendment/).

Six backlog-health analysis categories:
...
Principles: Read-first with surgical edits. Facts, not opinions. No false positives. Configurable scope. In Spec Amendment mode, suggest-only: the developer confirms every amendment via the implement coordinator; the agent never auto-rewrites specs on its own judgment.
